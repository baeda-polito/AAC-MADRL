...
class MARLISA(SAC):
    ...
    """
    def save_models(self, directory: str = 'marlisa_param_net'):
        print('.... SAVING MARLISA, mean and std ....')
        params = {"mean": [arr.tolist() for arr in self.norm_mean], "std": [arr.tolist() for arr in self.norm_std]}
        for i, pca in enumerate(self.pca):
            joblib.dump(pca, f'{directory}/pca_{i}.pkl')
        ...
        for i in range(len(self.action_dimension)):
            ...
            joblib.dump(self.state_estimator[i], f'{directory}/state_estimator_{i}.pkl')
        print('.... models saved ....')
    """
    def save_models(self, directory: str = 'marlisa_param_net'):
        print('.... SAVING MARLISA, mean and std ....')
        ...
        # 1) salva i PCA
        for i, pca in enumerate(self.pca):
            joblib.dump(pca, root / f'pca_{i}.pkl')

        # 2) crea cartella mean_std e scrivi config
        ...
        # 3) salva reti neurali e state estimators
        for i in range(len(self.action_dimension)):
            torch.save(self.soft_q_net1[i].state_dict(), root / f'soft_q_net1_{i}.pt')
            torch.save(self.soft_q_net2[i].state_dict(), root / f'soft_q_net2_{i}.pt')
            torch.save(self.target_soft_q_net1[i].state_dict(), root / f'target_soft_q_net1_{i}.pt')
            torch.save(self.target_soft_q_net2[i].state_dict(), root / f'target_soft_q_net2_{i}.pt')
            torch.save(self.policy_net[i].state_dict(), root / f'policy_net_{i}.pt')
            joblib.dump(self.state_estimator[i], root / f'state_estimator_{i}.pkl')

    print('.... models saved ....')
    def load_models(self, directory: str = 'marlisa_param_net'):
        print('.... loading models ....')
        for i in range(len(self.action_dimension)):
            self.pca[i] = joblib.load(f'{directory}/pca_{i}.pkl') 
            ...
            self.state_estimator[i] = joblib.load(f'{directory}/state_estimator_{i}.pkl')
