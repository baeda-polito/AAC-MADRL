...
class SAC(RLC):
    ...
    def set_encoders(self) -> List[List[Encoder]]:
        ...
        return encoders
    """
    def save_models(self, directory: str = 'sac_param_net'):
        print('.... saving models, mean and std ....')
        '''
        torch.save(self.norm_mean.state_dict(), f'{directory}/norm_mean.pt') #è sbagliato! sono liste di numeri non tensori, basta salverle in un file txt
                                                                             #o config (python file) come già avevamo discusso, così non credo funzionerà mai..
        torch.save(self.norm_std.state_dict(), f'{directory}/norm_std.pt')
        '''
        ...
        for i in range(len(self.action_dimension)):
            torch.save(self.soft_q_net1[i].state_dict(), f'{directory}/soft_q_net1_{i}.pt')
            torch.save(self.soft_q_net2[i].state_dict(), f'{directory}/soft_q_net2_{i}.pt')
            torch.save(self.target_soft_q_net1[i].state_dict(), f'{directory}/target_soft_q_net1_{i}.pt')
            torch.save(self.target_soft_q_net2[i].state_dict(), f'{directory}/target_soft_q_net2_{i}.pt')
    ...
    def save_models(self, directory: str = 'sac_param_net'):
        ...
        mean_std_dir = out_dir / 'mean_std'
        mean_std_dir.mkdir(parents=True, exist_ok=True)

        # Serialize normalization stats
        params = {
            "mean": [np.asarray(arr).tolist() for arr in self.norm_mean],
            "std":  [np.asarray(arr).tolist() for arr in self.norm_std],
        }

        # Write as JSON (portable & easy to read back)
        (mean_std_dir / 'config.json').write_text(json.dumps(params, indent=2))

        # Optional: also write a tiny Python file if some other code imports it
        (mean_std_dir / 'config.py').write_text(f"params = {params!r}\n")

        # Save per-action networks
        for i in range(len(self.action_dimension)):
            torch.save(self.soft_q_net1[i].state_dict(), out_dir / f'soft_q_net1_{i}.pt')
            torch.save(self.soft_q_net2[i].state_dict(), out_dir / f'soft_q_net2_{i}.pt')
            torch.save(self.target_soft_q_net1[i].state_dict(), out_dir / f'target_soft_q_net1_{i}.pt')
            torch.save(self.target_soft_q_net2[i].state_dict(), out_dir / f'target_soft_q_net2_{i}.pt')
            torch.save(self.policy_net[i].state_dict(), out_dir / f'policy_net_{i}.pt')

        print('.... models saved ....')
    def load_models(self, directory: str = 'sac_param_net'):
        print('.... loading models ....')
        ''''
        self.norm_mean.load_state_dict(torch.load(f'{directory}/norm_mean.pt')) #IDEM
        self.norm_std.load_state_dict(torch.load(f'{directory}/norm_std.pt'))  
        '''
        for i in range(len(self.action_dimension)):
            self.soft_q_net1[i].load_state_dict(torch.load(f'{directory}/soft_q_net1_{i}.pt', map_location=torch.device('cpu')))
            self.soft_q_net2[i].load_state_dict(torch.load(f'{directory}/soft_q_net2_{i}.pt', map_location=torch.device('cpu')))
            self.target_soft_q_net1[i].load_state_dict(torch.load(f'{directory}/target_soft_q_net1_{i}.pt', map_location=torch.device('cpu')))
            self.target_soft_q_net2[i].load_state_dict(torch.load(f'{directory}/target_soft_q_net2_{i}.pt', map_location=torch.device('cpu')))
