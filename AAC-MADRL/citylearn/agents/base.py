    ...
    def learn(self, episodes: int = None, deterministic: bool = None, deterministic_finish: bool = None, logging_level: int = None):
        ...
        for episode in range(episodes):
            ...
            if deterministic == False:
                #wandb.log({'Mean Reward_agent 0': rewards_summary['mean'][0]})
                if wandb.run is not None:
                    wandb.log({'Mean Reward_agent 0': rewards_summary['mean'][0],
                            'Mean Reward_agent 1': rewards_summary['mean'][1],
                            'Mean Reward_agent 2': rewards_summary['mean'][2],
                            'Mean Reward_agent 3': rewards_summary['mean'][3],
                            'Mean Reward_agent 4': rewards_summary['mean'][4],
                            'Mean Reward_agent 5': rewards_summary['mean'][5],
                            'Mean Reward_agent 6': rewards_summary['mean'][6],
                            'Mean Reward_agent 7': rewards_summary['mean'][7],
                            'Mean Reward_agent 8': rewards_summary['mean'][8],
                            })
            
                
            #wandb.log({'Mean Reward centralized': rewards_summary['mean']})
