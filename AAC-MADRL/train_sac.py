...
def parse_args():
    p = argparse.ArgumentParser(description="Train SAC on CityLearn in a portable way.")
...
def maybe_init_wandb(args, config):
    ...
    try:
        run_name = args.wandb_run_name or f"beta={args.beta}_sac_lr={args.lr}"
        run = wandb.init(
...
def main():
    ...
    models_dir = output_root / "save_models" / "sac" / f"beta={args.beta}" / f"lr={args.lr}"
